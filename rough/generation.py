...
from app.services.storage import store_submission
...
#         current_app.logger.info(f"Stored submission id={submission_id}")
#     except Exception:
#         current_app.logger.exception("Failed to store submission")
#         submission_id = None
...
#             s = int(q.get("score", 0))
#         except Exception:
#             logger.warning("Invalid score for question %r, treating as 0", q.get("id"))
#             s = 0
#         total_score += s

#     result = scoring.interpret_score(total_score)
